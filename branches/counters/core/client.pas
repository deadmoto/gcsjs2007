...
type
  ...
  TCData = packed record
    ...
    snpm:        TNumbTarifReal;//начисления по каждому тарифу по приборам учета
  ...
  public
    ...
    procedure CalcServ(s: integer);//расчёт начислений
  ...
  public
    constructor Create; overload;
    constructor Create(f, _npss: string; b: TDate; s, st, p: integer; mi: real; r: integer); overload;
    destructor Destroy; override;
    function Copy: TMan;
  end;
...
begin
  ...
  begin
    ...
    if cdata.mcount <> cdata.family.Count then
      ShowMessage(IntToStr(main.curregn));
    //
    SQL.Clear;
    SQL.Add('select *');
    SQL.Add('from sub');
...
begin
  ...
  for i := 0 to cdata.mcount - 1 do
  begin
    if ((cdata.f[i][serv] = 1) or (SearchOne(cdata.f, serv) = 0)) and (cdata.priv[i] <> 0) then
    begin //льгота на всю семью
      for j := 0 to cdata.mcount - 1 do
      begin
        if (cdata.sq[j][serv] > cdata.sq[i][serv]) or (cdata.pc[j][serv] = 0) then
          cdata.sq[j][serv] := cdata.sq[i][serv];
        if (cdata.pc[j][serv] < cdata.pc[i][serv]) and (cdata.sq[j][serv] = cdata.sq[i][serv]) then
        begin
...
begin
  ...
  begin
    case s of
      0:
      begin
        t1.Locate('id_cont', id, [loCaseInsensitive]);
        tc := t1;
      end;
      1:
      begin
        t2.Locate('id_rep', id, [loCaseInsensitive]);
        tc := t2;
      end;
      2:
      begin
        t3.Locate('id_cold', id, [loCaseInsensitive]);
        tc := t3;
      end;
      3:
      begin
        t4.Locate('id_hot', id, [loCaseInsensitive]);
        tc := t4;
      end;
      4:
      begin
        t5.Locate('id_canal', id, [loCaseInsensitive]);
        tc := t5;
      end;
      5:
      begin
        t6.Locate('id_heat', id, [loCaseInsensitive]);
        tc := t6;
      end;
      6:
      begin
        t7.Locate('id_gas', id, [loCaseInsensitive]);
        tc := t7;
      end;
      7:
      begin
        t8.Locate('id_el', id, [loCaseInsensitive]);
        tc := t8;
      end;
      12:
      begin
        t9.Locate('id_wood', id, [loCaseInsensitive]);
        tc := t9;
      end;
      13:
      begin
        t10.Locate('id_coal', id, [loCaseInsensitive]);
        tc := t10;
      end;
    end;
...
begin
  ...
  begin
    case s of
      0:
      begin
        t1.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t1;
      end;
      1:
      begin
        t2.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t2;
      end;
      2:
      begin
        t3.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t3;
      end;
      3:
      begin
        t4.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t4;
      end;
      4:
      begin
        t5.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t5;
      end;
      5:
      begin
        t6.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t6;
      end;
      6:
      begin
        t7.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t7;
      end;
      7:
      begin
        t8.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t8;
      end;
      12:
      begin
        t9.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t9;
      end;
      13:
      begin
        t10.Locate('id_' + nam, id, [loCaseInsensitive]);
        tc := t10;
      end;
    end;

    Result := tc.Fields[1].AsString;
  end;
end;
...
begin
  ...
  if cdata.rmcount = 1 then
    nv := '1';
  if cdata.rmcount = 2 then
    nv := '2';
  if (cdata.rmcount = 3) or ((cdata.rmcount > 4) and (cdata.quanpriv = cdata.rmcount)) then
    nv := '3';
  if cdata.rmcount = 4 then
    nv := '4';
  if cdata.rmcount >= 5 then
    nv := '5';
  ...
  if (cdata.rmcount = 1) and (cdata.mdd = 1) then
    nv := '6';
  if (cdata.rmcount > 1) and (cdata.mdd = 1) then
    nv := '7';
  try
...
begin
  ...
  begin
    ...
    quan := 0;
    if value2 < 0 then
      ShowMessage('Проверьте льготную площадь');
...
constructor TMan.Create(f, _npss: string; b: TDate; s, st, p: integer; mi: real; r: integer);
begin
  inherited Create;
  fio  := f;
  npss := _npss;
...
begin
  Result := TMan.Create(fio, npss, birth, pol, status, priv, mid, rel);
end;
