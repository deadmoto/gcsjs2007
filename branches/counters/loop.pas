...
type
  ...
  private
    ...
    values1,values2,values3: array of real;//начислени€ и субсиди€
    procedure SumV;
  public
    { Public declarations }
    status: integer;//0 - открыта дл€ просмотра, 1 - дл€ изменени€
  end;
...
//id_debt в запросах означает что запись в базе смотритьс€ только по ручному удержанию (занесенному через эту форму)

procedure TForm16.SumV;
{ ѕроцедура суммирует начислени€ и субсидию по всем услугам }
var
  v: real;
begin
...
{
  ѕроцедура сохран€ет изменени€, сделанные на форме, в служебную таблицу. “.о.
  таблица субсидий не мен€етс€, а изменени€ по€вл€ютс€ только в отчетах.
  ѕосле сохранени€ изменений форма закрываетс€, и открываетс€ форма текущих субсидий,
  на которой отображены последние изменени€.
}
var
  i, isFact: integer;
begin
...
{
  ѕроцедура устанавливает первоначальные настройки: €чейки закрыты дл€ редактировани€.
  ƒанные о начислени€х и субсидии загружаютс€ в €чейки. —огласно статусу формы
  гаситс€(0) или нет(1) кнопка "изменить".
}
var
  snpm,sub,pm: real;
  serv: integer;
...
{
  ѕроцедура обрабатывает нажатие CheckBox. ≈сли позвол€ет статус, то можно
  редактировать, а значит открываютс€ €чейки дл€ редактировани€.
}
begin
...
{
  ѕроцедура обрабатывает содержимое €чейки, устанавливает зап€тую, если надо, и
  суммирует начислени€ и считает субсидию.
}
begin
...
{
  ѕроцедура обрабатывает содержимое €чейки, устанавливает зап€тую, если надо, и
  суммирует начислени€ и считает субсидию, в случае нажати€ клавиши Enter.
}
begin
