...
type
  TForm30 = class(TForm)
    ...
    procedure LVHCompare(Sender: TObject; Item1, Item2: TListItem;
      Data: Integer; var Compare: Integer);
    procedure FormShow(Sender: TObject);
    procedure SGHSelectCell(Sender: TObject; ACol, ARow: Integer;
...
uses houses, datamodule, main, service;
...
procedure TForm30.LVHCompare(Sender: TObject; Item1,
...
procedure TForm30.SGHSelectCell(Sender: TObject; ACol,
...
begin
  if (SGH.RowCount>0)and(SGH.Cells[0,0]<>'') then begin
    Form24.status := 1;
    Form24.sec := status;
    Form24.ShowModal;
  end;
...
var
  i, c: integer;
begin
  for i:=0 to SGH.RowCount-1 do begin
    C := CompareText(SGH.Cells[0,i],s);
    if C>0 then begin //первая > второй
      InsertH(id,i);
      Result := i;
      break;
    end;
...
begin
  c := h[i1];
  cs := SGH.Cells[0,i1];
  for i:=Length(h)-1 downto i2+1 do begin
    h[i] := h[i-1];
...
var
  j: integer;
begin
  if SGH.RowCount>1 then begin
    for j:=i to SGH.RowCount-1 do begin
