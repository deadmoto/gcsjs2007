...
type
  ...
  TForm18 = class(TForm)
    ...
    procedure SGHKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses datamodule, loop,math, service, main, dateutils;
...
begin
  ...
  with Datamodule1.Query1 do begin
    ...
    if not eof then begin
      ...
      while not eof do begin
        for i:=0 to Length(cl.reg)-1 do
          if (FieldByName('regn').AsInteger = cl.reg[i]) then begin
  ...
  sum := 0;
  for i:=0 to Length(cl.reg)-1 do begin
...
procedure TForm18.SGHSelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
begin
  if Length(cl.reg)>0 then begin
