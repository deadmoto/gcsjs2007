...
uses datamodule, chhouse, sclient, main, service;
...
begin
  ...
  Combobox13.Clear;Combobox14.Clear;
  Edit3.Text := ''; Edit4.Text := '';CheckBox1.Checked := false;
end;
...
begin
  ...
  if ind <> -1 then begin
    ...
    if gas[Combobox7.ItemIndex]<>0 then begin
      combobox7.Enabled := true;
      if el[Combobox8.ItemIndex]<>1 then begin
        combobox8.Text := SelEl(1);
        Combobox8.OnChange(combobox8);
      end;
    end;
    if (gas[Combobox7.ItemIndex]=0)and combobox7.Enabled then begin
...
begin
  ...
  if ind <> -1 then begin
    ...
    if el[Combobox8.ItemIndex]=1 then begin//газовая
      combobox7.Enabled := true;
      if gas[Combobox7.ItemIndex]=0 then begin
    ...
    if (el[Combobox8.ItemIndex]<>1)and combobox8.Enabled then begin
      combobox7.Enabled := false;
      if gas[Combobox7.ItemIndex]<>0 then begin
...
var
  maxid, n: integer;
begin
  if not CardEmpty then begin
    if not ExistHouse(n) then begin
...
begin
  if not CardEmpty then begin
    n := 0;
    if not ExistHouse(n) or ExistHouse(n) and (n=Form30.house) then begin
      Datamodule1.Database1.StartTransaction;
      try
      with Datamodule1.Query1 do begin
        Close;
...
procedure TForm24.Edit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
